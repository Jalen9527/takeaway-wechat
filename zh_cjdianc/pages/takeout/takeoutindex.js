...
Page({
    ...
    scsj: function(t) {
        var a = this, e = "2" == this.data.issc ? "1" : "2", s = wx.getStorageSync("users").id, o = a.data.store_id;
    ...
    Coupons: function() {
        var s = this, t = wx.getStorageSync("users").id, a = s.data.store_id;
        app.util.request({
    ...
    spxqck: function(t) {
        ...
        var a = t.currentTarget.dataset.itemIndex,	 //当前商品的下标id
			e = t.currentTarget.dataset.parentindex,  //父极的下标id
		...
		//接口获取二维码
		this.getWechatCode(t.currentTarget.dataset.goodid);
		wx.hideToast();
    },
	getWechatCode: function(goodid) {
		wx.showToast({title: '加载中', icon: 'loading', duration: 10000});
		var _this = this;
		app.util.request({
            url: "entry/wxapp/getMiniCode",
            data: {
                page: 'zh_cjdianc/pages/takeout/takeoutindex',
				pid: goodid,
				type_id:_this.data.store_id,
            },
            success: function(t) {
				console.log("获取二维码接口======" ,t);
				var qrcode_url = t.data.qrcode_url;
                _this.setData({
                     qrcodeUrl: qrcode_url
                })
				console.log('qrcodeUrl==============', _this.data.qrcodeUrl);
				_this.getImage(qrcode_url);
            }
        });
	},
	getImage: function(url){
		...
		wx.downloadFile({ //下载二维码
			url: _this.data.qrcodeUrl, //图片地址
			success (res) {
				console.log('下载二维码url================',res.tempFilePath);
				_this.setData({
					qrcodeUrl:res.tempFilePath, //将二维码放到本地临时地址
				})
				//关闭loading
				wx.hideToast();
			}
    ...
    gwcadd: function(t) {
        var i = this, r = this.data.dishes, n = t.currentTarget.dataset.goodid, a = Number(t.currentTarget.dataset.num) + 1, e = t.currentTarget.dataset.id;
        wx.showLoading({
            title: "正在加载",
            mask: !0
        }), app.util.request({
    ...
    cartdec: function(t) {
        for (var a = t.currentTarget.dataset.itemIndex, e = t.currentTarget.dataset.parentindex, i = this.data.dishes, s = this.data.cart_list.res, r = t.currentTarget.dataset.goodid, n = this, o = this.data.dishes[e].good[a], d = (wx.getStorageSync("users").id, 
        this.data.store_id, 0), c = s.length; d < c; d++) if (s[d].good_id == r) {
            var l = 1;
            Number(o.start_num) == Number(s[d].num) && (l = Number(o.start_num));
            var g = Number(s[d].num) - l, u = s[d].id;
            wx.showLoading({
                title: "正在加载",
                mask: !0
            }), app.util.request({
    ...
    cartadd: function(t) {
        var a = t.currentTarget.dataset.itemIndex, e = t.currentTarget.dataset.parentindex, i = this.data.dishes, s = this.data.cart_list.res, r = t.currentTarget.dataset.goodid, n = this, o = this.data.dishes[e].good[a], d = wx.getStorageSync("users").id, c = this.data.store_id, l = "1" == getApp().xtxx.hygn && this.data.iszk && "0.00" != o.vip_money ? 1 : 0, g = 1;
        "0" == o.start_num || n.isInArray(s, r) || (g = Number(o.start_num)), console.log(g), 
        wx.showLoading({
            title: "正在加载",
            mask: !0
        }), app.util.request({
    ...
    attrClick: function(t) {
        var a = this, e = this.data.gginfo.good_id, s = t.target.dataset.groupId, o = t.target.dataset.id, i = a.data.gg;
    ...
    ggaddcart: function() {
        var t = this.data.itemIndex, a = this.data.parentindex, i = this.data.dishes, r = (this.data.dishes[a].good[t], 
        this.data.cart_list.res, this), n = this.data.gginfo, e = wx.getStorageSync("users").id, s = this.data.gg, o = this.data.store_id, d = [], c = !0;
        for (var l in s) {
            var g = !1;
    ...
    gwcreload: function() {
        this.data.dishes;
        var a = this, t = wx.getStorageSync("users").id, e = this.data.store_id;
        app.util.request({
    ...
    scrolltolower: function() {
        var o = this.data.dishes, i = this, e = wx.getStorageSync("users").id, s = i.data.store_id, r = this.data.loadindex;
    ...
    selectMenu: function(s) {
        var o = this.data.dishes, i = this, r = wx.getStorageSync("users").id, n = i.data.store_id, d = s.currentTarget.dataset.itemIndex;
        if (i.setData({
    ...
    clear: function() {
        var i = this, r = this.data.dishes, a = wx.getStorageSync("users").id, e = i.data.store_id;
...
        });
	...
	canvasImage(g) {  //分享画布
		...
		var _this = this;
		var logoImg = g.currentTarget.dataset.logo;
		var goodid = g.currentTarget.dataset.goodid;
		var goodInfo = g.currentTarget.dataset;
		...
		wx.downloadFile({ //下载图片
		  ...
		  success (res) {
		    ...
		    if (res.statusCode === 200) {
				...
				a = screen.screenWidth * 0.8
				_this.setData({
					cwidth: a,
					cheight:1.778 * a  //1.778 * a
				}); 
				
				//设置画布矩形的，大小
				context.fillRect(0, 0, a, 1.778 * a);
				// context.fillRect(0, 0, 500, 1000 );
				// console.log('size:::' ,a , a*1.778);
				console.log('filePath::', downimgPath);
				//设置文字字体大小，颜色，背景色是白色，默认颜色是白色
				context.drawImage(
					downimgPath,
					0,
					0,
				...
				context.fillText('长按可以保存或分享海报', 0.31* a, 1.72 * a)
				
				//商品名称
				context.setFontSize(16)
				context.setFillStyle('#16191d')
				context.fillText("商品名：" + goodInfo.name, 0.05* a, 1.08* a)
				
				//价格
				context.setFontSize(14)
				context.setFillStyle('#ff4544')
				context.fillText("价格：￥" + goodInfo.money, 0.05* a, 1.16* a)
				
				//月销
				context.setFillStyle('#9a9a9a')
				context.fillText("月销 " + goodInfo.sales + " 笔", 0.05* a, 1.22* a)
				
				// if(goodInfo.content.length > 0) {
				// 	context.setFontSize(16)
				// 	context.setFillStyle('#ffffff')
				// 	context.fillText(goodInfo.content, 0.03* a, 0.97* a)
				// }
				
				//放置二维码
				context.drawImage(_this.data.qrcodeUrl, 0.3 * a, 1.25 * a, 0.4 * a, 0.4 * a);
				// wx.createCanvasContext("mycanvas")中id值与页面中不匹配时，将不会触发
				...
				context.draw(false, (callback) => {
					wx.canvasToTempFilePath({ //canvas 生成图片 生成临时路径
						...
						success: function(res) {
							...
							wx.hideToast();
							console.log('海报res' , res.tempFilePath)
						}
					})
				});
    ...
    onLoad: function(e) {
        ...
        app.util.request({
            ...
            success: function(t) {
                console.log(t.data);
                var a = t.data.store.time, e = t.data.store.time2, s = t.data.store.time3, o = t.data.store.time4, i = t.data.store.is_rest;
                console.log("当前的系统时间为" + l), console.log("商家的营业时间从" + a + "至" + e, s + "至" + o), 
                1 == i ? (c.setData({
                    yysjtoggle: !1
                }), console.log("商家正在休息" + i)) : console.log("商家正在营业" + i), a < o ? a < l && l < e || s < l && l < o || s < l && o < s ? (console.log("商家正常营业"), 
                c.setData({
                    time: 1
                    ...
                    time: 3,
                    yysjtoggle: !1
                })) : o < a && (a < l && l < e || s < l && o < l || l < s && l < o ? (console.log("商家正常营业"), 
                c.setData({
                    time: 1
                })) : l < a || e < l && l < s ? (console.log("商家还没开店呐，稍等一会儿可以吗？"), c.setData({
                    time: 2,
		...
		c.getGoodInfo(goodid);
    },
    maketel: function(t) {
        var a = this, e = this.data.store.tel, s = this.data.paytel;
    ...
    onShareAppMessage: function() {
		...
		var params;
		if( this.data.share.goodid > 0 ) {
			params = "&itemIndex=" + this.data.share.itemIndex + "&parentindex=" + this.data.share.parentindex + "&goodid=" + this.data.share.goodid;
		}
		console.log(params);
        return {
            title: this.data.store.name,
            path: "/zh_cjdianc/pages/takeout/takeoutindex?storeid=" + this.data.store_id + params,
            success: function(t) {},
            fail: function(t) {}
        };
    }
